...
Se ejecuto una prueba de rendimiento contra el endpoint de listado de tareas usando el despliegue con Nginx (HTTPS).

**Comando ejecutado**:

```bash
ab -n 200 -c 10 -k https://localhost/api/tasks
```

- `-n 200`: 200 peticiones en total.
- `-c 10`: 10 peticiones simultaneas (concurrencia).
- `-k`: Mantener conexiones abiertas (Keep-Alive).
...
Requests per second:    278.96 [#/sec] (mean)
...
Connect:        0    0   1.7      0      13
Processing:     2   35  40.3      5      97
Waiting:        2   35  40.4      5      97
Total:          2   35  40.3      5      97
...
|---------|-------|---------------|
| Peticiones/segundo | 278.96 | Buen rendimiento para un entorno de desarrollo |
| Tiempo medio por peticion | 35.8 ms | Aceptable, incluye overhead de TLS y proxy |
| Peticiones fallidas | 0 | Sin errores, la aplicacion es estable |
| Tiempo maximo | 97 ms | Las primeras peticiones son mas lentas (warmup de JVM) |
| Mediana (50%) | 5 ms | La mayoria de peticiones son muy rapidas |
